...
contract VaultTest is Test {
    ...
    function test_scenario() public {
        ...
        assertEq(usdc.balanceOf(alice), 1_333_333); // artinya uang yang diambil sama alice sama dengan 1_333_333
        assertEq(vault.balanceOf(alice), 0);        // artinya deposit alice yang di dalam vault sudah habis
        vm.stopPrank();

        // bob withdraw 2_000_000
        vm.startPrank(bob);
        vault.withdraw(2_000_000);
        assertEq(usdc.balanceOf(bob), 2_666_666);   // artinya uang yang diambil sama bob sama dengan 2_666_666
        assertEq(vault.balanceOf(bob), 0);          // artinya deposit bob yang di dalam vault sudah habis
        vm.stopPrank();
    }
}